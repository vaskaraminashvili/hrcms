@php
    ...
    $depthClass = match (true) {
        ...
        $depth === 3 => 'dept-tree-node--depth-3',
        $depth === 4 => 'dept-tree-node--depth-4',
        $depth === 5 => 'dept-tree-node--depth-5',
        default => 'dept-tree-node--depth-6',
    };

    $type = $record->type;
    $typeLabel = $type ? $type->getLabel() : null;
    $filamentColor = $type ? $type->getColor() ?? 'info' : 'info';
    $typeColor = \App\Support\DepartmentBadgeColors::treePaletteKey(
        is_string($filamentColor) ? $filamentColor : 'gray',
    );

    $status = $record->status;
    $isActive = $status === \App\Enums\DepartmentStatus::ACTIVE;
    ...
    $descendantCounts = app(
        \App\Services\DepartmentDescendantTypeCountService::class,
...
>
    ...
    <div class="dept-tree-node__row" x-bind:class="{ 'dept-tree-node__row--collapsed': !open }" x-on:click="toggle()">
        ...
        <div class="dept-tree-node__chevron-wrap">
            @if ($hasChildren)
                <span class="dept-tree-node__chevron" data-tree-chevron
                    x-bind:class="{ 'dept-tree-node__chevron--collapsed': !open }" data-tree-toggle
        ...
        {{-- Status dot --}}
        <span class="dept-tree-node__status-dot"
            x-tooltip="@js($isActive ? __('filament.resources.departments.active') : __('filament.resources.departments.archived'))"
        ...
        {{-- Name (link) --}}
        <a href="{{ $positionsUrl }}" target="_blank" rel="noopener noreferrer" class="dept-tree-node__name"
            x-on:click.stop>
        ...
        {{-- Type badge --}}
        @if ($typeLabel)
            <div class="dept-tree-node__type-badge-wrap">
                <span class="dept-tree-node__type-badge-label">{{ __('filament.resources.departments.type') }}:</span>
        ...
        @if (!empty($descendantCounts))
            <div class="dept-tree-node__counts scrollbar-style-3" x-on:click.stop>
        ...
        <div class="dept-tree-node__actions" x-on:click.stop>
            <a href="{{ $createUrl }}" class="dept-tree-node__action-btn"
            ...
            <a href="{{ $editUrl }}" class="dept-tree-node__action-btn"
    ...
    {{-- Children --}}
    @if ($hasChildren)
        <div class="dept-tree-node__children" data-tree-children x-show="open"
            x-transition:enter="dept-tree-node__children--entering"
            x-transition:leave="dept-tree-node__children--leaving" role="group">
            @foreach ($children->sortBy('order') as $child)
                <x-filament.resources.departments.components.tree-node :record="$child" :depth="$depth + 1"
